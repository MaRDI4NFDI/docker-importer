...
class Arxiv:
    ...
    def disambiguate_autor(self, name: str) -> Author:
        ...
        while not finish:
            ...
            try:
                author_html = (
                    soup.find("div", id="content").find("h1").get_text(strip=True)
                )
    ...
    def arxiv_api(arxiv_id: str) -> FeedParserDict:
        ...
        log.debug(
            f"Fetching arXiv entry for {clean_id} (original: {arxiv_id}) - using: {full_url}"
        )
        ...
        if not feed.entries:
            log.warning(
                f"No arXiv entries found for ID: {clean_id}. The list is empty."
            )
            return None
...
class ArxivPublication:
    ...
    def create(self):
        ...
        if self.title != "Error":
            ...
            if self.title:
                item.descriptions.set(
                    language="en", value="scientific article from arXiv"
                )
        ...
        if self.QID:
            log.debug(
                f"arXiv preprint with arXiv id: {self.arxiv_id} created with ID {self.QID}."
            )
            return self.QID
        else:
            log.debug(
                f"arXiv preprint with arXiv id: {self.arxiv_id} could not be created."
            )
            return None
