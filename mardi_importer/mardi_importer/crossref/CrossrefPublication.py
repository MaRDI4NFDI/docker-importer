...
class CrossrefPublication:
    ...
    def __post_init__(self):
        ...
        if self.QID:
            ...
            try:
                ...
                log.error(
                    f"Publication with doi: {self.doi} not found in Crossref: {str(e)}"
                )
                return None
            else:
                        ...
                        if "created" in metadata.keys():
                            if "date-parts" in metadata["created"].keys():
                                if len(metadata["created"]["date-parts"][0]) > 1:
                                    self.proceedings_month = str(
                                        metadata["created"]["date-parts"][0][1]
                                    )
                ...
                if "author" in metadata.keys():
                    for author in metadata["author"]:
                        if "given" in author.keys() and "family" in author.keys():
                            author_label = (
                                f"{author['given'].title()} {author['family'].title()}"
                            )
    ...
    def create(self):
        ...
        if self.title:
            ...
            if self.description:
                item.descriptions.set(language="en", value=self.description)

            if self.instance:
                self.QID = item.is_instance_of_with_property(
                    self.instance, "wdt:P356", self.doi
                )
            ...
            if self.identical:
                ...
                existing_item = item.is_instance_of_with_property(
                    self.instance, "wdt:P356", self.identical
                )
                if existing_item:
            ...
            if self.journal:
                ...
                self.QID = item.is_instance_of_with_property(
                    self.instance, isbn_prop_nr, self.isbn
                )
            ...
            if len(self.day) > 0:
                item.add_claim(
                    "wdt:P577",
                    f"+{self.year}-{self.month}-{self.day}T00:00:00Z",
                    precision=11,
                )
            elif len(self.month) > 0:
                item.add_claim(
                    "wdt:P577", f"+{self.year}-{self.month}-00T00:00:00Z", precision=10
                )
    ...
    def __preprocess_journal(self):
        ...
        if journal_id:
            ...
            if len(self.issn_print) > 0:
                ...
                claims.append(
                    self.api.get_claim(
                        "wdt:P236", self.issn_print, qualifiers=qualifier
                    )
                )
            if len(self.issn_online) > 0:
                ...
                claims.append(
                    self.api.get_claim(
                        "wdt:P236", self.issn_online, qualifiers=qualifier
                    )
                )
    ...
    def __preprocess_book(self, container_book):
        ...
        item.descriptions.set(language="en", value="academic book")
    ...
    def __preprocess_proceedings(self):
        ...
        if proceedings_id:
            ...
            if len(self.proceedings_month) > 0:
                item.add_claim(
                    "wdt:P577",
                    f"+{self.proceedings_year}-{self.proceedings_month}-00T00:00:00Z",
                )
