# ---------------------------------------------------------------------------
# Deploy the full-import flow to Prefect on Kubernetes.
#
# Usage (from a machine that can reach the Prefect server):
#
#   export PREFECT_API_URL=http://prefect-mardi.zib.de/api
#   export PREFECT_API_AUTH_STRING=admin:xxx
#   python prefect_workflow/prefect_deploy_full_import.py
#
...
#      run hasn't finished").
#   2. Deploys the flow from the Git repo with a weekly cron schedule.
...
if __name__ == "__main__":
    ...
    flow.from_source(
        ...
        # Adjust cron expression or set via the Prefect UI later.
        cron="0 2 * * 0",
    )

    print("\n✔  Deployment 'full-import/prefect-full-import' created.")
    print("   Schedule : every Sunday 02:00 UTC")
