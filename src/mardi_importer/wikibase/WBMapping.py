...
def get_wbs_local_id(wbs_original_id):
    ...
    engine = db.create_engine(f'mysql+mysqlconnector://{db_user}:{db_pass}@{db_host}/{db_name}')
    connection = engine.connect()
    metadata = db.MetaData()
    try: 
        wbs_entity_mapping = db.Table('wbs_entity_mapping', metadata, autoload=True, autoload_with=engine)
        query = db.select([wbs_entity_mapping.columns.wbs_local_id]).where(wbs_entity_mapping.columns.wbs_original_id == bytes(wbs_original_id,"utf-8") )
